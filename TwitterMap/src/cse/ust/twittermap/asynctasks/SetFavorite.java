...
public class SetFavorite extends AsyncTask<Status, Void, Boolean> {
	...
	public SetFavorite(Context context, Fragment fragment, TweetData localData, int index){
	...
	protected void onPreExecute() {
		postDialog = ProgressDialog.show(context, "Status", "Favoring...", true, false);
	}
	...
	protected void onPostExecute(Boolean result) {
		postDialog.dismiss();
		if (result == true)
			localData.setFavorite(index, true);
		else
	...
	protected Boolean doInBackground(twitter4j.Status... statuses) {
		try {
			TwitterFactory.get4jInstance().createFavorite(statuses[0].getId());
			Thread.sleep(3000);
		} catch (Exception e) {
			return false;
		}
